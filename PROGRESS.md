...
完整版见方案 §12.4。按性价比排序，**建议顺序 E1 → E6 → E3 →（E2/E4 并行）**：

| # | 实验 | 为什么 | 成本 |
|---|---|---|---|
| **E1** | **打开节流闸门**（`interval=4` + 阈值 0.05） | R0 把闸门**全关了**，跑的是最贵配置；打开即降本 | **零代码** |
| **E6** | **用更全的打分**（two → all query） | 打分只占 1.9 %，却因只读 2 个位置损失 18 % 保真度 | 小（改开关） |
| **E3** | **headroom-gated 启用** | 静态差才开，是 §17.2 结论的直接推论 | 小 |
| **E2** | **运行时实现 EMA 平滑** | 离线 recall 打平但更平滑（搬运更少）；**离线测过、运行时从未实现** | 小 |
| **E4** | **消掉未被计时的 host 侧开销**（先补计时再优化） | R1 后 refresh 已只剩 22 ms/轮，**≈34 ms/轮的 host 开销成了最大单项** | 中 |
| **E5** | **索引化 KV（PagedAttention 式）** | 每轮搬运 **57 MB → 0.45 MB（↓128×）** | 大（需新核） |
| **E7** | **换判别标准**（注意力 × value 范数等） | 原始注意力大小 ≠ 草稿真正需要什么 | 中 |
