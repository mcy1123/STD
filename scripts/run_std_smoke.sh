...
conda run -n specvlm python scripts/benchmark_std.py \
  --model-path "$ROOT/models/Qwen2.5-VL-7B-Instruct" \
  --dataset VideoDetailCaption \
  --data-path "$ROOT/datasets/VideoDetailCaption" \
  --eval-num 1 \
  --frame-num 32 \
  --max-new-tokens 64 \
  --gamma 9 \
  ...
  --strict-equality \
  --output "$ROOT/results/std_qwen2_5_vl_7b/smoke_videodetailcaption.jsonl"
