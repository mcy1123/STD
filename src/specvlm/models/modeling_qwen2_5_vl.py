...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if hasattr(self, 'sparse_kv_selection') and self.sparse_kv_selection is not None:
            sparse_visual_indices = self.sparse_kv_selection['visual_indices']
            text_start_idx = self.sparse_kv_selection['text_start_idx']
            total_kv_len = key_states.shape[2]

            # Build per-head sparse indices: top-K visual + all text/generated tokens
            all_text_indices = torch.arange(text_start_idx, total_kv_len,
                                            device=key_states.device)
            sparse_keys = []
            sparse_values = []
            for h in range(key_states.shape[1]):  # num_kv_heads
                vis_idx = sparse_visual_indices[h].to(key_states.device)
                all_idx = torch.cat([vis_idx, all_text_indices])
                sparse_keys.append(key_states[:, h:h+1, all_idx, :])
                sparse_values.append(value_states[:, h:h+1, all_idx, :])
            key_states = torch.cat(sparse_keys, dim=1)
            value_states = torch.cat(sparse_values, dim=1)

            # Build new zero-filled attention mask for sparse KV (all past
            # positions are visible during single-token autoregressive decoding).
            attention_mask = torch.zeros(
                bsz, 1, q_len, key_states.shape[2],
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    # cache_position=cache_position, #MODIFIED
                    position_embeddings=position_embeddings,
                )
