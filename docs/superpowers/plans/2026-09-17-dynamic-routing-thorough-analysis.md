...
按"性价比"排序；每项给出**目的 / 做法 / 判据**。E1–E3 都是低成本，**建议先做**。

| # | 实验 | 做法 | 判据 | 成本 |
|---|---|---|---|---|
| **E1** | **打开节流闸门** | `--selection-update-interval 4` + `--min-selection-change-ratio 0.05`（R0 这次**全关**，跑的是最贵配置） | 搬运时间下降 ≥ 60 %，且 Δaccept 不劣于 R0 | **零代码**，改配置 |
...
**建议顺序**：**E1 → E6 → E3 →（E2/E4 并行）**，把 E5/E7 留作后续。
