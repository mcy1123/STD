...
## 12. 为什么动态没赢：成本核算与指标错配（2026-09-18）

> 本节回答一个此前从未量化的核心问题：**拦路虎到底是"动态这个想法"还是"实现开销"？**
> 结论：**两者都有，但根因是指标错配**——见 §12.2。

### 12.1 成本核算（R0 实测，10 样本 × 128 tokens）

| 项目 | 耗时 | 占总解码 | 平均每轮 |
|---|---:|---:|---:|
| **打分**（读注意力、算打分表，`collect`） | **1.87 s** | **1.9 %** | **11 ms** |
| **搬运**（把新选中的画面块写进简化缓存，`refresh`） | **11.84 s** | **11.7 %** | **71 ms** |
| 调试校验 T1（**本次实验专用，正式跑没有**） | ≈ 17.3 s | — | — |
| 其余（未逐项归因） | ≈ 7 s | — | — |
| **static 基线** | 62.8 s（168 轮） | | 374 ms/轮 |
| **dynamic 合计** | **101.0 s**（167 轮） | | |

**关键换算**：

- 动态每轮自带 **82 ms（11 + 71）** 的过路费 = **单轮成本的 22 %**。
  → **动态必须把轮数减少 22 % 以上才可能回本。**
- 一次搬运要搬 **57 MB**（28 层 × 4 头 × 996 块 × 128 维 × 2 B × K/V），实测 **71 ms** →
  **有效带宽仅 ≈ 0.8 GB/s**，而 A100 显存带宽约 **2000 GB/s**，**差约 2500 倍**。
  → 瓶颈**不是带宽**，而是 **224 次 `torch.unique` + 56 次 gather/copy 的碎片化发射 + 2 次强制 `cuda.synchronize()`**。
- 若把搬运完全去掉，过路费从 **82 ms/轮（22 %）降到 11 ms/轮（3 %）**——**准入门槛降低约 7 倍**。

> ⚠️ 本批样本中轮数几乎没变（168 → 167）：**多花的 38 s 几乎全是纯开销，没有换回任何速度**。
> 但这不是"开销是主因"，而是本批静态本来就做得不错（见 §12.2）。

### 12.2 指标错配 —— 最关键的一条

把 L1 的三个选法放在一起看：

| 选法 | 召回率 vs oracle | 注意力覆盖 | 每轮接受词数（proxy） |
|---|---:|---:|---:|
| Static | 0.6465 | 0.3713 | 7.45 |
| Previous（我们的动态） | 0.7288 | 0.3920 | 7.87（**+0.42**） |
| **Oracle（上限）** | **1.0000** | 0.4311 | **8.65（+1.20）** |

**静态只拿到理想的 65 %** → **"静态 top-K 就是最优"是错的，"动态选 KV"的前提成立，不是伪命题。**

但出现了一个表面矛盾：**静态召回率只有 0.647，接受率却高达 0.83~1.0。** 原因是两个指标的敏感度差得极远：

- **召回率**要求选中的 KV 集合与理想集合高度重合 —— 很苛刻；
- **接受率**只要求选中的 KV "够好到让草稿吐出同样的词" —— **很多不同的 KV 集合都会产生同样的 greedy 输出**。

> **换 KV 这件事，在召回率上有很大空间，在接受率上却常常没有空间。**

这一条同时解释了三件事：
1. 为什么静态召回率平庸但接受率很高；
2. 为什么动态的 +8 pp 召回率提升传不到接受率；
3. 为什么只有当静态**真的选得很差**时，这点提升才终于变成接受率提升（§11 的分层结果）。

**潜力转化率**：accept-proxy 口径下，我们只拿到 `0.42 / 1.20 = 35 %` 的 oracle 红利（召回率口径 23 %）。

### 12.3 预测器为什么弱（三个具体机制）

| 机制 | 实测证据 |
|---|---|
| **信息过期快** | 相邻两轮选择重合度仅 **Jaccard 0.58** —— 上一轮的信息约一半已作废 |
| **估计器有噪声** | 只读 2 个位置的注意力，保真度 **0.815**（损失约 18 %） |
| **每次改动太猛** | 每轮直接替换约 **30 %** 的 KV，是硬替换、无平滑 |

### 12.4 下一阶段实验计划（L2.5 / L3）

**E1 与 E6 已于 2026-09-19 执行完毕**（A100 单变量 2×2，结果见 `PROGRESS.md` §17.8）。剩余项按性价比排序：

| # | 实验 | 做法 | 判据 | 成本 | 状态 |
|---|---|---|---|---|---|
| **E1** | **打开节流闸门** | `--selection-update-interval 4` + `--min-selection-change-ratio 0.05`（R0 这次**全关**，跑的是最贵配置） | 搬运时间下降 ≥ 60 %，且 Δaccept 不劣于 R0 | **零代码** | ✅ **通过**：Δ **+0.031（v2）/ +0.045（v3）**；改选比例 0.467→0.136，refresh 86.5→15.7 ms/轮 |
| **E6** | **用更全的打分** | `--dynamic-collector v3`（= all-query，**不需要新代码**） | `fidelity` 0.815 → ~1.0；Δaccept 有可测提升 | **零代码** | ⚠️ **单独无效（−0.003）**，仅在节流下转正（+0.011）→ 保真度不是瓶颈 |
| **E4′** | **消掉未被计时的 host 侧开销** | 先给动态主循环补上覆盖 host 侧的计时，定位后再优化；原 E4 的"去同步 / 合并 unique / 批量 gather"仍是候选手段 | 过路费从 ~75–160 ms/轮 降到 < 35 ms/轮 | 中 | ⏭ **优先级第一** |
| **E3** | **headroom-gated 启用** | 先跑一轮 static 量出该样本表现，低于阈值 τ 才启用动态 | 在低 headroom 分层上 Δaccept ≥ +0.10，同时高分层不再亏 | 小（§11 结论的直接推论） | ⏭ 次之 |
| **E2** | **运行时实现 EMA 平滑** | 把 `S_t = TopK(A_t)` 换成 `S_t = TopK(EMA(A))`；离线已测 λ=0.5 的召回率 **0.7294 vs Previous 0.7288（打平）**，但更平滑 | 相同 Δaccept 下 churn 更低（搬运更少）；或 churn 相同下 Δaccept 更高 | 小（**离线已测过，运行时从未实现**） | 待定 |
| **E5** | **索引化 KV（PagedAttention 式）** | 不做物理复制，让稀疏注意力按索引直接从完整缓存取 K/V；"换选择"退化为一组索引写入 | 每轮搬运数据量 **57 MB → 0.45 MB（↓128×）**；过路费 → < 5 % | **大**（需新注意力核） | 后续 |
| **E7** | **换判别标准** | 用"注意力 × value 范数"或按草稿实际需要标定，而不是原始注意力大小 | 同等 K 下 accept 提升 | 中 | 后续（E6 已证"更准"不等于"更有用"） |

**顺序修订（2026-09-19）**：原计划 **E1 → E6 → E3 →（E2/E4 并行）** 已执行完前两项。
实测结果是 **E1 是主效应、E6 单独无效**，且节流后 **refresh 只剩 ~16 ms/轮、过路费却仍有上百 ms/轮**
（`PROGRESS.md` §17.8.3 第 7 条）——所以新顺序为 **E4′ → E3 →（E2/E5/E7 视情况）**：
**先查清钱花在哪，再谈挑得更聪明。**

> **基线铁律（2026-09-19 新增）**：**配置对照必须在同一台机器、同一 session 内现测基线。**
> 同一份代码、同一批样本，static 接受率在 A6000 与 A100 之间单样本最大差 **0.151**、均值只差 **0.006**，
> 拿跨硬件的旧结果当基线会把硬件效应藏进每一个 Δ 里。证据见 `docs/experiment-provenance.md` §7。

### 12.5 天花板与定位（不要高估）

- **即使完美预测（拿到 oracle 上限），也只有约 +1.20 词/轮 ≈ 16 % 的轮数节省。**
- 因此这条线是**中等收益**，**不是"翻倍"**；它的价值必须与 §12.1 的**开销削减叠加**才有意义。
- 反过来说：**"选得更好"和"换得更便宜"是同一个乘积的两项**，只做一项都很难看到端到端收益。
