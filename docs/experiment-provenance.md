...
## 8. 【2026-09-19】加速比口径：仓库里**没有** 3× 的 static decode 加速

**结论先说**：论文自己写的是 **up to 1.94× wall-time**（Table 1：LLaVA-OneVision **1.72/1.82/1.83/1.59×**，
Qwen2-VL **1.94/1.71/1.62/1.70×**，batch size 8 + 专用 top-K 核）。**我们实测的 static decode 加速是
1.1–2.0×/样本、1.36–1.75×/聚合。** 仓库里确实存在 3 个 >2.5× 的数字，但**它们全部是坏基线**。

### 8.1 各来源的 static-vs-AR decode 加速（逐样本口径）

| 来源 | 样本 | 每样本比值 均值 / 最大 |
|---|---:|---|
| A100，2026-09-09 各 run | 1–3 | **1.14–1.27× / 1.39×** |
| A6000，R0（full） | 10 | 1.48× / 1.96× |
| A6000，R1（incremental） | 10 | 1.62× / 1.98× |
| A100，2026-09-19 A/B/C/D | 10 | 1.43–1.87× / 2.00–**3.51×** |

### 8.2 那 3 个 >2.5× 是**被拖慢的 AR 基线**，不是更快的 static

| run / 样本 | static decode | **AR decode** | 比值 |
|---|---:|---:|---:|
| C / 647-1 | 3.91 s | **13.71 s**（同批其他样本 7.5 s） | 3.51× |
| D / 504-2 | 4.30 s | **13.53 s** | 3.14× |
| C / 717-1 | 4.01 s | **10.64 s** | 2.65× |

同一个 run 里，**AR 的 decode 对其余样本稳定在 7.52–7.62 s**；上面这三处的 static 时间（3.91 / 4.30 / 4.01）
和别的 run 完全一致（3.90 / 4.30 / 3.75）。**差的是分母，不是分子。**
它们全部来自 `timing_valid=false` 的 run（共租户占卡），本就不该用于任何墙钟结论。

### 8.3 两个必须同时声明的口径

1. **ratio-of-sums ≠ mean-of-ratios。** A100 10 样本批：ratio-of-sums 是 **1.36–1.75×**，
   而 mean-of-ratios 是 **1.43–1.87×**——后者被少数比值大的样本拉高。
   `scripts/benchmark_std.py:660` 用的是 **mean-of-ratios**（`sum(total_speedups)/len(...)`），
   会系统性偏高，引用该脚本的输出时要标明口径；`summarize_a100_dynamic.py` 用的是 ratio-of-sums。
2. **decode ≠ 端到端。** decode 只算生成循环；端到端含 prefill，A100 上是 **1.03–1.08×**
   （见 §2）。两者差一个量级，混用会得出互相矛盾的结论。

**记录规则**：任何加速比必须同时给出 ①硬件/session ②ratio-of-sums 还是 mean-of-ratios
③decode 还是端到端 ④`timing_valid`。
